<x-rolemanager::layouts.app>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-rolemanager::card>
                <form action="{{route('rolemanager.roles.update',$role->id)}}" method="POST">
                    @csrf
                    @method('put')
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <x-rolemanager::input-label for="name" value="Role Name" />
                            <x-rolemanager::text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name',$role->name)" required autofocus autocomplete="off" />
                            ...
                            @foreach($permissions as $permission)
                                ...
                                @endif
                                    <div class="border-l pl-1">
                                        <label>
                                            <input type="checkbox" name="permission_ids[]" value="{{$permission->id}}" {{$role->permissions()->where('permissions.id',$permission->id)->exists() ? 'checked' : ''}}> 
                                            {{ucfirst($temp[1])}}
                                        </label>
                                    </div>
